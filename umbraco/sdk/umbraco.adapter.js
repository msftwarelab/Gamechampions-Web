import { BASE_URL, URL_REGEX } from "./umbraco.constants";
...
export const toResponsiveImage = images => {
  if (images && images.length > 0) {
    ...
    let responsiveImageData = {
      ...
      src: `${process.env.STORAGE_URL}${defaultImage.imageUrl}`,
      alt: defaultImage.alternateText,
      srcset: `${process.env.STORAGE_URL}${defaultImage.imageUrl} ${defaultImage.width}w`
    };
    // If there any images left over, append their contents to the srcset
    for (let i = 1; i < images.length; i++) {
      responsiveImageData.srcset = `${responsiveImageData.srcset}, ${process.env.STORAGE_URL}${images[i].imageUrl} ${images[i].width}w`;
    }
